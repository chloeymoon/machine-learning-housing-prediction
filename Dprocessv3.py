...
def dataProcess(dataframe):
    ...
    # # Simplifying Some Categorical dataframe
    ...
    # # Further Simplification on Some Categorical dataframe
    ...
    # dataframe['Fireplaces'] = dataframe['Fireplaces'].apply(
    # lambda x: 1 if x > 0 else 0)
    dataframe['PoolArea'] = dataframe['PoolArea'].apply(
        lambda x: 1 if x > 0 else 0)
    dataframe['has2ndfloor'] = dataframe['2ndFlrSF'].apply(
        lambda x: 1 if x > 0 else 0)
    dataframe['hasgarage'] = dataframe['GarageArea'].apply(
        lambda x: 1 if x > 0 else 0)
    dataframe['hasbsmt'] = dataframe['TotalBsmtSF'].apply(
        lambda x: 1 if x > 0 else 0)
    dataframe['hasfireplace'] = dataframe['Fireplaces'].apply(
        lambda x: 1 if x > 0 else 0)
    ...
    dataframe['Total_ Bath'] = dataframe['BsmtFullBath'] + (.5 * dataframe['BsmtHalfBath']) + \
        dataframe['FullBath'] + (.5 * dataframe['HalfBath'])

    dataframe['TotalSF'] = dataframe['TotalBsmtSF'] + dataframe['1stFlrSF'] + dataframe['2ndFlrSF']

    dataframe['Total_sqr_footage'] = (dataframe['BsmtFinSF1'] + dataframe['BsmtFinSF2'] +
    ...
    dataframe['Total_porch_sf'] = (dataframe['OpenPorchSF'] + dataframe['3SsnPorch'] +
                              dataframe['EnclosedPorch'] + dataframe['ScreenPorch'] +
    ...
    # dataframe['Total_Interior_SF'] = dataframe['TotalBsmtSF'] + dataframe['1stFlrSF'] + dataframe['2ndFlrSF']
